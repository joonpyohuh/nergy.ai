...
export type StakeholderRole = 'overview' | 'marketer' | 'designer' | 'developer' | 'operator'

export const STAKEHOLDER_ROLES: StakeholderRole[] = ['overview', 'marketer', 'designer', 'developer', 'operator']
...
export interface NodeRoleExplanations {
  marketer: RoleExplanation
  designer: RoleExplanation
  developer: RoleExplanation
  operator: RoleExplanation
}
...
export interface LogicNode {
  ...
  outputs: string[]
  evidence: Evidence
  roleExplanations: NodeRoleExplanations
}

export type SerializableLogicNode = Omit<LogicNode, 'icon'>

export type RelationshipType = 'data' | 'event' | 'decision' | 'control' | 'handoff' | 'feedback'
...
export interface LogicEdge {
  ...
  risks: string[]
  evidence: Evidence
  documentationOpportunities: string[]
}
...
export const NODE_COLORS = ['#3182F6', '#8B5CF6', '#F59E0B', '#14B8A6', '#EC4899', '#64748B', '#EF4444', '#0EA5E9']
...
export const logicNodes: LogicNode[] = [
  {
    ...
    color: '#3182F6',
    stage: '입력',
    inputs: ['채널별 고객 메시지', '주문·계정 이벤트', '채널 메타데이터(채널, 시각, 기기)'],
    outputs: ['정규화된 고객 신호', '고객·세션 식별자', '문제 시작점 이벤트'],
    evidence: 'DOCS',
    roleExplanations: {
      marketer: {
        summary: '모든 채널의 고객 목소리가 한 지점으로 모이는 입구예요.',
        whyItMatters: '캠페인·채널별 반응을 같은 기준으로 비교할 수 있어 유입 품질과 문의 유형을 분석할 수 있어요.',
        keyQuestions: ['채널별 문의 유형 리포트는 어디서 보나요?', '캠페인 태그나 UTM이 신호에 함께 붙나요?'],
        expectedOutputs: ['채널별 문의량 지표', '캠페인 연계 문의 태그'],
      },
      designer: {
        summary: '고객이 어떤 채널에서 말을 걸어도 같은 대화로 인식돼요.',
        whyItMatters: '첫 응답 대기, 수신 확인 같은 초기 상태를 채널별로 다르게 설계해야 해요.',
        keyQuestions: ['수신 확인·입력 중 상태를 어떻게 표현하나요?', '채널별 입력 제약(길이·첨부)은 무엇인가요?'],
      },
      developer: {
        summary: '채널별 이벤트를 하나의 신호 스키마로 정규화하는 단계예요.',
        whyItMatters: 'payload 필드 정의, 중복 이벤트, 재시도 규칙이 전체 연동의 안정성을 좌우해요.',
        keyQuestions: ['이벤트 스키마와 필수 필드는 무엇인가요?', '중복 수신과 순서 뒤바뀜은 어떻게 처리하나요?'],
        expectedOutputs: ['채널 이벤트 계약', '신호 정규화 스키마'],
      },
      operator: {
        summary: '문의가 유실 없이 접수되는 첫 관문이에요.',
        whyItMatters: '누락된 신호는 곧바로 CS 사고로 이어지므로 수신 모니터링 기준이 필요해요.',
        keyQuestions: ['수신 실패 알림은 누구에게 전달되나요?', '피크 시간대 처리 한도와 대기 정책은요?'],
      },
    },
  },
  {
  ...
  },
  {
    ...
    color: '#EF4444',
    stage: '이관',
    inputs: ['중단 이유·신뢰도 점수', '전체 대화·행동 맥락'],
    outputs: ['담당자 티켓', '사람의 결정 결과'],
    evidence: 'DOCS',
    roleExplanations: {
      marketer: {
        summary: '민감한 고객이 사람의 손길을 받는 순간이에요.',
        whyItMatters: 'VIP·이탈 위험 고객의 이관 경험이 브랜드 신뢰에 큰 영향을 줘요.',
        keyQuestions: ['VIP 고객은 이관 우선순위가 다른가요?', '이관 후 고객 만족도는 따로 측정되나요?'],
      },
      designer: {
        summary: 'AI에서 사람으로 넘어가는 전환 경험을 설계하는 단계예요.',
        whyItMatters: '“상담원에게 연결합니다” 순간의 대기·기대 설정이 경험을 좌우해요.',
        keyQuestions: ['이관 대기 중 고객에게 어떤 상태를 보여주나요?', '사람이 이어받았음을 어떻게 표시하나요?'],
      },
      developer: {
        summary: '티켓 생성과 맥락 전달 페이로드가 정의되는 단계예요.',
        whyItMatters: '전달 필드가 부족하면 담당자가 대화를 처음부터 다시 읽어야 해요.',
        keyQuestions: ['이관 페이로드의 필수 필드는요?', '외부 지원 도구 연동 시 상태 동기화는요?'],
        expectedOutputs: ['이관 계약서(payload 명세)'],
      },
      operator: {
        summary: '이관 조건과 담당자 배정 규칙을 운영하는 단계예요.',
        whyItMatters: '이관 기준이 느슨하면 상담 폭주, 엄격하면 미해결 누적이 생겨요.',
        keyQuestions: ['이관 조건(신뢰도·금액·감정)은 무엇인가요?', '이관 티켓의 처리 SLA는요?'],
      },
    },
  },
]

export const logicEdges: LogicEdge[] = [
  {
    id: 'signal-memory',
    source: 'signal',
    target: 'memory',
    ...
    transferredData: ['고객 ID', '채널', '현재 메시지', '세션 ID', '이벤트 시각'],
    successCondition: '고객 식별이 완료되고 맥락 조회 요청이 접수된다.',
    risks: ['고객 식별 실패로 신규 고객으로 오인', '식별자 매칭 지연으로 첫 응답 지연'],
    evidence: 'DOCS',
    documentationOpportunities: ['Customer context retrieval', 'Identity matching', 'Memory fallback behavior'],
  },
  {
    id: 'memory-rules',
    source: 'memory',
    ...
    transferredData: ['고객 맥락 요약', '과거 이슈 이력', '선호·감정 신호', 'VIP·등급 정보'],
    successCondition: '맥락이 반영된 정책 판단이 시작된다.',
    risks: ['오래된 맥락으로 잘못된 정책 적용', '맥락 조회 실패 시 기본 정책만 적용'],
    evidence: 'DOCS',
    documentationOpportunities: ['Context-to-policy mapping', 'Stale memory handling'],
  },
  {
    ...
    transferredData: ['행동 방향 결정', '보상·권한 한도', '적용된 규칙 버전', '승인 필요 여부'],
    successCondition: '한도와 규칙을 준수하는 실행 계획이 생성된다.',
    risks: ['규칙 충돌 시 의도와 다른 계획 생성', '규칙 버전 불일치로 재현 불가'],
    evidence: 'DOCS',
    documentationOpportunities: ['Actionbook 작성 가이드', 'Rule precedence reference'],
  },
  {
    ...
    transferredData: ['응답 콘텐츠', '대상 채널', '발송 우선순위', '사건 상태'],
    successCondition: '고객 채널로 응답이 발송되고 사건 상태가 갱신된다.',
    risks: ['채널 포맷 제약으로 콘텐츠 누락', '발송 실패 시 사건 상태 불일치'],
    evidence: 'DOCS',
    documentationOpportunities: ['채널 전환 행동 명세', 'Delivery failure handling'],
  },
  {
    ...
    transferredData: ['계획된 행동 내역', '금액·한도 정보', '사용한 규칙·데이터 근거'],
    successCondition: '검사 결과(통과/차단/보류)가 실행 전에 반환된다.',
    risks: ['검사 지연으로 응답 시간 증가', '검사 우회 경로 존재 가능성'],
    evidence: 'CONFIRM',
    documentationOpportunities: ['Guardrail evaluation spec', 'Sensitive action catalog'],
  },
  {
    id: 'trust-channel',
    source: 'trust',
    ...
    transferredData: ['승인된 행동', '검사 통과 기록', '대체 응답(차단 시)'],
    successCondition: '승인된 행동만 고객에게 실행된다.',
    risks: ['차단 시 고객 응답 지연', '대체 응답 품질 저하'],
    evidence: 'CONFIRM',
    documentationOpportunities: ['Blocked action UX copy', 'Approval latency SLA'],
  },
  {
    id: 'trust-handoff',
    source: 'trust',
    ...
    transferredData: ['전체 대화 기록', '실행된 행동과 결과', '중단 이유', '준비된 결정 근거'],
    successCondition: '담당자가 대화를 다시 읽지 않고 결정할 수 있는 티켓이 생성된다.',
    risks: ['맥락 필드 누락으로 담당자 재조사 필요', '이관 폭주 시 대기 시간 증가'],
    evidence: 'DOCS',
    documentationOpportunities: ['AI → Human 이관 계약서', 'Escalation criteria reference'],
  },
  {
    id: 'channel-signal',
    source: 'channel',
    ...
    transferredData: ['고객 후속 메시지', '기존 사건 ID', '채널 전환 정보'],
    successCondition: '후속 메시지가 새 문의가 아니라 같은 사건으로 연결된다.',
    risks: ['사건 연결 실패로 대화 맥락 초기화', '채널 전환 시 중복 사건 생성'],
    evidence: 'DOCS',
    documentationOpportunities: ['Session continuity rules', 'Cross-channel case linking'],
  },
  {
    id: 'handoff-trust',
    source: 'handoff',
    ...
    transferredData: ['사람의 결정(승인/거절/수정)', '결정 사유', '처리 시간'],
    successCondition: '사람의 결정이 행동 기록과 평가 데이터에 반영된다.',
    risks: ['결정 사유 미입력으로 개선 근거 부족', '피드백 반영 지연'],
    evidence: 'CONFIRM',
    documentationOpportunities: ['Human decision feedback loop', 'Reviewer guidelines'],
  },
]
